...
BaseCodeGenerator::BaseCodeGenerator(const std::string& name,
                                     ...
                                     ASTNode* module, SymtableVisitor* symtab,
                                     int lineno, CompileInfo* info)
    : CodeBuilder(name, context, symtab->find_scope(module), lineno, info)
{
...
void BaseCodeGenerator::make_call(int n, const std::vector<ASTNode*>& args,
                                  const std::vector<KeywordNode*>& keywords)
{
    int arg = args.size() + n;
    for (auto& arg : args)
        arg->visit(this);
...
ASTNode* BaseCodeGenerator::visit_expr(ExprNode* node)
{
    ...
    if ((value->get_tag() != NodeType::NT_NUMBER) &&
        (value->get_tag() != NodeType::NT_STRING)) {
        value->visit(this);
        emit_op(POP_TOP);
    }
...
void BaseCodeGenerator::make_closure(mtpython::interpreter::PyCode* code,
                                     int args,
...
ASTNode* BaseCodeGenerator::visit_functiondef(FunctionDefNode* node)
{
    ...
    FunctionCodeGenerator sub_gen(node->get_name(), context, node, symtab,
                                  node->get_line(), compile_info);
...
ASTNode* BaseCodeGenerator::visit_classdef(ClassDefNode* node)
{
    ...
    ClassCodeGenerator sub_gen(node->get_name(), context, node, symtab,
                               node->get_line(), compile_info);
    PyCode* code = sub_gen.build();
...
BaseCodeGenerator::visit_lambda(mtpython::tree::LambdaNode* node)
{
    set_lineno(node->get_line());

    LambdaCodeGenerator sub_gen("<lambda>", context, node, symtab,
                                node->get_line(), compile_info);
    PyCode* code = sub_gen.build();
...
ASTNode* BaseCodeGenerator::visit_while(WhileNode* node)
{
    ...
    if (test_const == 0) {
        if (ASTNode* orelse = node->get_orelse()) visit_sequence(orelse);
    } else {
        CodeBlock* end = new_block();
...
ModuleCodeGenerator::ModuleCodeGenerator(mtpython::vm::ThreadContext* context,
                                         mtpython::tree::ASTNode* module,
                                         SymtableVisitor* symtab,
                                         CompileInfo* info)
    : BaseCodeGenerator("module", context, module, symtab, -1, info)
{
    compile(module);
}
...
void ModuleCodeGenerator::compile(ASTNode* module) { module->visit(this); }

AbstractFunctionCodeGenerator::AbstractFunctionCodeGenerator(
    const std::string& name, mtpython::vm::ThreadContext* context,
    mtpython::tree::ASTNode* tree, SymtableVisitor* symtab, int lineno,
    CompileInfo* info)
    : BaseCodeGenerator(name, context, tree, symtab, lineno, info)
{}
...
FunctionCodeGenerator::FunctionCodeGenerator(
    ...
    mtpython::tree::ASTNode* tree, SymtableVisitor* symtab, int lineno,
    CompileInfo* info)
    : AbstractFunctionCodeGenerator(name, context, tree, symtab, lineno, info)
{
    compile(tree);
}
...
LambdaCodeGenerator::LambdaCodeGenerator(const std::string& name,
                                         mtpython::vm::ThreadContext* context,
                                         mtpython::tree::ASTNode* tree,
                                         SymtableVisitor* symtab, int lineno,
                                         CompileInfo* info)
    : AbstractFunctionCodeGenerator(name, context, tree, symtab, lineno, info)
{
    compile(tree);
...
ClassCodeGenerator::ClassCodeGenerator(const std::string& name,
                                       mtpython::vm::ThreadContext* context,
                                       mtpython::tree::ASTNode* tree,
                                       SymtableVisitor* symtab, int lineno,
                                       CompileInfo* info)
