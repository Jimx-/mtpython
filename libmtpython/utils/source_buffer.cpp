...
SourceBuffer::SourceBuffer(const string& source,
...
SourceBuffer::SourceBuffer(vector<char>::iterator first,
                           vector<char>::iterator last)
    : buf(first, last), fname("<no file>")
{
    pos = 0;
}
...
mtpython::parse::SourceType SourceBuffer::get_src_type() { return src_type; }
...
int SourceBuffer::tell_pos() { return pos; }

void SourceBuffer::seek(int pos) { this->pos = pos; }

bool SourceBuffer::eof() { return (pos == buf.size()); }

int SourceBuffer::size() { return buf.size(); }

vector<char>::iterator SourceBuffer::begin() { return buf.begin(); }

vector<char>::iterator SourceBuffer::end() { return buf.end(); }

std::string SourceBuffer::get_line(int line_nr)
{
    vector<char>::iterator it;
    string ret = "";
    for (it = begin() + line_offset[line_nr];
         it < begin() + line_offset[line_nr + 1]; it++)
        /* no trailing CRLF */
        if (*it != '\n' && *it != '\r') ret += *it;
