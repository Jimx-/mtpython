...
static std::string argcount_err_msg(const std::string& name, int nargs,
                                    int nkwargs, Signature& sig)
{
    int n = sig.get_nargs();
    ...
    return string_format("%s() takes %d positional argument%s but %d %s given",
                         name.c_str(), n, plural, (nargs + nkwargs), plural2);
}

/* TODO: support keyword only argument(PEP 3102) */
void Arguments::parse(const std::string& fname, M_BaseObject* first,
                      Signature& sig, std::vector<M_BaseObject*>& scope,
                      const std::vector<M_BaseObject*>& defaults) const
{
            ...
            if (index < input_argcount) {
                if (index != -1)
                    throw InterpError::format(
                        ...
                        fname.c_str(), keyword_name.c_str());
            } else {
        ...
        if (remaining_kwargs > 0) {
            if (wrapped_kwargs) {
                for (std::size_t i = 0; i < keywords.size(); i++) {
                    bool matched = false;
                    for (std::size_t j : kwarg_index)
                        if (matched = (i == j)) break;
                    if (!matched) { /* extra keyword */
                        space->setitem_str(wrapped_kwargs, keywords[i],
                                           keyword_values[i]);
                    }
                }
            } else {
                throw InterpError::format(
                    space, space->TypeError_type(),
                    "%s() got %d unexpected keyword argument%s", fname.c_str(),
                    remaining_kwargs, (remaining_kwargs > 1) ? "s" : "");
            }
        }
    }
