...
M_BaseObject* BuiltinCode4::funcrun_obj(ThreadContext* context, M_BaseObject* func, M_BaseObject* obj, Arguments& args)
{
	ObjSpace* space = context->get_space();
	Function* as_func = dynamic_cast<Function*>(func);
	if (!as_func)
	...
	return context->pop_local_frame(this->func(context, scope[0], scope[1], scope[2], scope[3]));
}
...
InterpFunctionWrapper::InterpFunctionWrapper(const std::string& name, InterpFunction4 f)
{
	code = new BuiltinCode4(name, f);
}

InterpFunctionWrapper::InterpFunctionWrapper(const std::string& name, InterpFunction4 f, const Signature& sig)
{
...
M_BaseObject* GatewayCache::build(M_BaseObject* key)
{
	...
	Function* f = new(space->current_thread()) Function(space, wrapper->get_code());
