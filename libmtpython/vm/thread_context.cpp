...
void LocalFrame::new_local_ref(M_BaseObject* obj) { local_refs.insert(obj); }

void LocalFrame::delete_local_ref(M_BaseObject* obj) { local_refs.erase(obj); }
...
ThreadContext::~ThreadContext() { SAFE_DELETE(compiler); }
...
M_BaseObject* ThreadContext::pop_local_frame(M_BaseObject* result)
{
    ...
    if (result) {
        LocalFrame* outer_frame =
            local_frame_stack[local_frame_stack.size() - 2];
