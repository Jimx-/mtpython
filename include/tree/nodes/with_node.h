...
public:
	...
	void set_context_expr(ASTNode * context_expr) { this->context_expr = context_expr; }
	ASTNode * get_optional_vars() { return optional_vars; }
	void set_optional_vars(ASTNode * optional_vars) { this->optional_vars = optional_vars; }
	...
	virtual void print(const int padding) {
		...
		if (optional_vars) {
			std::cout << blank << "  " << line << ": Optional Vars:" << std::endl;
...
public:
	...
	~WithNode()
	{
		...
		for(std::size_t i = 0; i < items.size(); i++) SAFE_DELETE(items[i]);
	}
	...
	virtual void print(const int padding) {
		...
		for (auto item : items) item->print(padding + 4);
