...
public:
	...
	~ImportFromNode() { for (std::size_t i = 0; i < names.size(); i++) SAFE_DELETE(names[i]); }
	...
	virtual void print(const int padding) {
		...
		std::cout << blank << "  " << line << ": Module: " << module << std::endl;
		std::cout << blank << "  " << line << ": Names:" << std::endl;
		for (unsigned int i = 0; i < names.size(); i++) names[i]->print(padding + 4);
